...
\vskip.10in
\noindent Given this relation:

\begin{quote}
$Orders(cust_id, cust_name, address, telephone, email, order_number, order_date, item_number, item_name, selling_price, quantity)$
\end{quote}

This relation stores a history of orders made by a customer. An order could possibly contain more than one item.

\begin{quote}
\item $i.$ What are the functional dependencies for this relation?
\item $ii.$ Decompose this relation into smaller relations with minimal redundancy and show that the resulting decomposition the original relation's functional dependencies.
\end{quote}
